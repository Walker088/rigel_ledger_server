...
func (d *UserDao) GetComplete(userId string) (*pojo.UserInfo, error) {
	...
	queryLedgerSummary := `
    SELECT
        CASE x.main_currency
            WHEN 'USD' THEN x.bl_assets
            ELSE x.bl_assets * (SELECT exchange_rate FROM ref_exchange_rates WHERE from_code = 'USD' AND to_code = x.main_currency)
        END bl_assets,
        CASE x.main_currency
            WHEN 'USD' THEN x.bl_liabilities
            ELSE x.bl_liabilities * (SELECT exchange_rate FROM ref_exchange_rates WHERE from_code = 'USD' AND to_code = x.main_currency)
        END bl_liabilities,
        CASE x.main_currency
            WHEN 'USD' THEN (x.bl_assets - x.bl_liabilities)
            ELSE (x.bl_assets - x.bl_liabilities) * (SELECT exchange_rate FROM ref_exchange_rates WHERE from_code = 'USD' AND to_code = x.main_currency)
        END bl_networth,
        CASE x.main_currency
            WHEN 'USD' THEN COALESCE(x.ie_monthly_income, 0)
            ELSE COALESCE(x.ie_monthly_income, 0) * (SELECT exchange_rate FROM ref_exchange_rates WHERE from_code = 'USD' AND to_code = x.main_currency)
        END ie_monthly_income,
        CASE x.main_currency
            WHEN 'USD' THEN COALESCE(x.ie_monthly_expense, 0)
            ELSE COALESCE(x.ie_monthly_expense, 0) * (SELECT exchange_rate FROM ref_exchange_rates WHERE from_code = 'USD' AND to_code = x.main_currency)
    ...
    FROM (
        SELECT
            ...
            (SELECT main_currency FROM user_info WHERE user_id = $1)
        FROM 
    ...
    SELECT
        ...
        CASE currency
            ...
            ELSE amount_origin 
                / (SELECT exchange_rate FROM ref_exchange_rates WHERE from_code = 'USD' AND to_code = currency)
                * (SELECT exchange_rate FROM ref_exchange_rates WHERE from_code = 'USD' AND  to_code = main_currency)
        END amount_adjusted
    FROM (
        SELECT
            j.transac_id,
            ...
            (SELECT main_currency  FROM user_info WHERE user_id = j.user_id),
...
func (d *UserDao) Update(userId string, u *pojo.UpdateUserInfo) (*pojo.UpdateUserInfo, error) {
	var updated *pojo.UpdateUserInfo
	updateUserQuery := `
    UPDATE user_info
    SET 
        user_name  = $1,
